...
Resend의 무료 한도와 정책은 바뀔 수 있으므로 실제 배포 전에 Resend 대시보드에서 현재 플랜과
사용량을 확인합니다. 유료 플랜으로 자동 변경되는 설정은 사용하지 않습니다.
...
- 받은 메일함, 보낸 메일함, 임시보관함, 휴지통은 사용자별로 분리됩니다.
- 메일을 휴지통에서 복원하면 이동 전 메일함으로 돌아갑니다.
- 첨부 파일은 해당 메일을 볼 수 있는 사용자만 내려받을 수 있습니다.
- 외부 발송 실패 메일은 보낸 메일함에서 실패 상태로 표시되며 다시 발송할 수 있습니다.
- 기밀 및 최고기밀 메일은 기본 설정에서 외부 주소로 보낼 수 없습니다.
- 외부 수신은 수신 도메인과 웹훅 설정이 모두 완료되어야 동작합니다.
...
5. 발송 실패를 시험한 뒤 보낸 메일함의 재시도 기능과 상태 변경을 확인합니다.
